...
def cntry_to_ctn(country):
    countries = [
        ...
        {'timezones': ['America/Noronha', 'America/Belem', 'America/Fortaleza', 'America/Recife', 'America/Araguaina',
                       ...
                       'America/Rio_Branco'], 'code': 'BR', 'continent': 'South America', 'name': 'Brasil',
        ...
        {'timezones': ['America/New_York', 'America/Detroit', 'America/Kentucky/Louisville',
         ...
         'continent': 'North America', 'name': 'USA', 'capital': 'Washington, D.C.'},
        ...
        {'timezones': ['Asia/Hong_Kong'], 'code': 'HK', 'continent': 'Asia', 'name': 'Hong Kong',
         'capital': 'Hong Kong'},
    ...
    for values in countries:
        if values['name'] == country:
            return values['continent']
    return "None"


def define_region(conn, row):
    latitude = row[2]
    longitude = row[3]
    geolocator = Nominatim(user_agent="CarExchange")
    coordString = "" + latitude + " , " + longitude
    location = geolocator.reverse(coordString)
    country = location.address.split(',')[-1][1:]
    print(country)
    continent = cntry_to_ctn(country)
    return continent

def new_entries(conn):
    cur = conn.cursor()
    cur.execute("SELECT * FROM INCOMING_DATA LIMIT 1")
    rows = cur.fetchall()
    for row in rows:
        print(row[1:])
        continent = define_region(conn, row)
        print(continent)
        if continent == 'Europe':
            cur2 = conn.cursor()
            try:
                cur2.execute("INSERT INTO EU (username,latitude,longitude,acceptance_distance,time_start,time_end, type) VALUES (?,?,?,?,?,?,?)", row[1:])
                cur2.execute("DELETE FROM INCOMING_DATA WHERE NUMBER = ?",(row[0:1]))
            except sqlite3.IntegrityError:
                print('ERROR')
        elif continent == 'Asia':
            cur2 = conn.cursor()
            try:
                cur2.execute("INSERT INTO ASIA (username,latitude,longitude,acceptance_distance,time_start,time_end, type) VALUES (?,?,?,?,?,?,?)", row[1:])
                cur2.execute("DELETE FROM INCOMING_DATA WHERE NUMBER = ?",(row[0:1]))
            except sqlite3.IntegrityError:
                print('ERROR')
            ...
            cur2 = conn.cursor()
            try:
                cur2.execute("INSERT INTO NA (username,latitude,longitude,acceptance_distance,time_start,time_end, type) VALUES (?,?,?,?,?,?,?)", row[1:])
                cur2.execute("DELETE FROM INCOMING_DATA WHERE NUMBER = ?",(row[0:1]))
            except sqlite3.IntegrityError:
                print('ERROR')
            ...
            cur2 = conn.cursor()
            try:
                cur2.execute("INSERT INTO SA (username,latitude,longitude,acceptance_distance,time_start,time_end, type) VALUES (?,?,?,?,?,?,?)", row[1:])
                cur2.execute("DELETE FROM INCOMING_DATA WHERE NUMBER = ?",(row[0:1]))
            except sqlite3.IntegrityError:
                print('ERROR')
        else:
            cur2 = conn.cursor()
            try:
                cur2.execute("INSERT INTO OTHER (username,latitude,longitude,acceptance_distance,time_start,time_end, type) VALUES (?,?,?,?,?,?,?)", row[1:])
                cur2.execute("DELETE FROM INCOMING_DATA WHERE NUMBER = ?", (row[0:1]))
            except sqlite3.IntegrityError:
                print('ERROR')
